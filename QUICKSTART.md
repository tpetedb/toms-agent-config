...
| Put a question, step or approval on the owner's list | nothing by hand; the sections and the fixed text of the page are [`.human/todo.toml`](.human/todo.toml) | none | `just human ask --kind question --topic governance --title "..." --question "..." --recommendation "..."` (`--kind step` for a one-time task), then `just human render --check`. After editing `.human/todo.toml`: `just human render` |
| Ask for and give an approval | `.human/approvals/<id>.json`, written by `tac human ask --kind approval` | none | `just human ask --kind approval --tool ... --arguments '{...}' --revision <sha> --pr <n>` binds the action, its arguments and the revision. Only the owner's approving review on that pull request, or `just approve <id>` (owner only, on the host; refused in a sandbox or an agent session), counts; a ticked box does not. `just human verify <id>` exits 0 approved, 3 waiting, 4 rejected, 1 refused |
