...
**Gate children.** A gate runs code a builder wrote: the root justfile's recipe bodies, an order's criteria as shell strings, the tests `just verify` starts. The runner never runs that code with access to its store. Every child that touches the candidate tree, the gate itself and the runner's own `git status` (a repository's config can make it start an fsmonitor or a clean filter), starts under a macOS Seatbelt profile from `/usr/bin/sandbox-exec`, named by absolute path. The profile denies reading and writing the controller store subtree, the signing key by name, connecting to any socket in the store, and writing any folder above the store, since Seatbelt matches paths at access time and a renamed parent would otherwise expose the key under a new path. The child gets a fixed environment: the runner's own PATH and `HOME`, `USER`, `LOGNAME`, `LANG`, `LC_ALL`, `LC_CTYPE`, `TMPDIR`, nothing else, so no token, no `GIT_*` and no `TAC_STATE_HOME`. It runs in the checkout at the committed revision the receipt names; a gate that leaves HEAD moved or the tree changed gets no receipt. Where no sandbox exists (not macOS, or `sandbox-exec` missing) the runner refuses to gate or probe and says why; it never falls back to running unsandboxed. The same user can still read the key file from outside a sandbox, for instance from a Claude hook, which runs unsandboxed; the keychain entry of M3 (C6), or a dedicated user, closes that.
...
- C3: the skeleton's receipts bind repository, revision, run, stage and policy hash; `ci.yml` reads `runner.pub` from the base revision or a separately provisioned repository setting, never from the candidate; a replayed receipt, a receipt for another revision and a candidate-supplied key are each rejected; the runner signs only its own observations. The gate child is isolated (section 10, gate children): it runs under a Seatbelt profile that denies the controller store, the key, the runner socket and renaming any folder above them, with a scrubbed environment, and the runner refuses to gate where no sandbox exists; `test_runner_skeleton.py` proves a criterion that reads or writes the key, moves the store, reaches the socket or reads a token fails, that `git status` runs sandboxed, and that the unsupported platform refuses. What holds at M0: no code a builder wrote, run by the runner, can read or change the key or the store. What waits for M3 (C6): the key is still a file the owner's user can read from any unsandboxed process, such as a Claude hook or another terminal, and the runner's own memory is protected only by macOS's debugger rules; the keychain entry, or a dedicated macOS user, closes both, and the live proof of M4 checks it.
