...
- C3: the skeleton's receipts bind repository, revision, run, stage and policy hash; `ci.yml` reads `runner.pub` from the base revision or a separately provisioned repository setting, never from the candidate; a replayed receipt, a receipt for another revision and a candidate-supplied key are each rejected; the runner signs only its own observations.
