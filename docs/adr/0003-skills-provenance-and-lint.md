...
- The lint reads every text file of a skill, whatever its suffix. A pictograph is every Symbol, Other character but box drawing, plus the emoji parts that are not: the variation selector, the zero-width joiner, the enclosing keycap, the skin tone modifiers and the tag characters. `--help` runs on every `.py`, `.sh` or executable file at any depth, leaving out only a non-executable file under `examples/`.
- One file outside the M4 order changed: `work-order/SKILL.md` gained `metadata.author: the owner`, because the provenance rule holds every skill, that one included. Nothing else in it changed. It is recorded here and not in `docs/DESIGN.md`, whose section 12 paragraph stays exactly as the order's shared edit gives it.
