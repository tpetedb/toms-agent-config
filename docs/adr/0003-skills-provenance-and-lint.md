...
We will hold every skill to the portable subset with `tests/test_skills.py`, run as `just skills-lint`: the specification's limits and fields, string metadata, provenance present, a body under 500 lines, `--help` exiting 0 from every bundled script, no em dash and no pictograph, and the sum of names and descriptions under 8,000 characters. The test's docstring cites the two pages and the date, so the number is checked again when a client changes it.
