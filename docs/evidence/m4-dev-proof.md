...
- A policy tamper written through a shell is refused at commit only where the policy schema holds the value: dropping `.agents/**` from `deny_write` is refused by `tac check --staged` and by `tac sync` alike. Loosening `deny_read` is refused at commit only while the lock is stale; after a `tac sync` it commits, since nothing pins `deny_read` below the project, so it is a reviewable configuration change caught at layer 3b, the owner's CODEOWNERS entry on `.agents/`. A floor pin for `deny_read` is follow-up config; no new key lands in this order. `test_a_resynced_loosening_of_deny_read_is_left_to_codeowners` holds the gap and fails once the pin lands.
